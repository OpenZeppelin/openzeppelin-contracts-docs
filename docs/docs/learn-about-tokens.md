...
- [ERC20Mintable](https://github.com/OpenZeppelin/openzeppelin-solidity/blob/master/contracts/token/ERC20/ERC20Mintable.sol) — `ERC20Mintable` allows users with the [`MinterRole`](/api/docs/learn-about-access-control.html) to mint tokens to users using the `mint()` function. Minting can also be finished, locking the `mint()` function's behavior.
...
And now the minters in my `minters` array (like DOGGO Network multisig) can mint tokens to the dogsitters in exchange for watching the nice doggos while their owners leave for vacation. The token is `ERC20Burnable` because they might have to burn the tokens for warmth after entrusting their livelihood to our DOGGO token network.
...
We'll these contracts tokenize the time of our dogsitters. When a dogsitter wants to sell an hour of their time to watch a dog, they can mint an ERC721 token that represents that hour slot. Then they can sell this token on an exchange and go to the owner's house at the right time to watch their dogs.
